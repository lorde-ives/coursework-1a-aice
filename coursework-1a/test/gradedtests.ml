...
(* Steps the x86 simulator for [n] steps starting from machine state
   [m].  The test passes if the resulting state satisfies the
   predicate [f].  [s] is an error message describing the expected
...
(* Runs an x86 program [p] until it returns a result, which is
   expected to be [ans]
 *)
